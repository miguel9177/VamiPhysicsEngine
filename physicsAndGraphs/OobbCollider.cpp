...
OobbCollider::OobbCollider(glm::vec3 size_, glm::vec3 offsetOfColliderPosition_, glm::vec3 offsetOfColliderRotation_)
{
	size = size_;
...
//this will update the tranformations of the visual of the collider
void OobbCollider::updateModelMatrix(unsigned int modelMatLoc, float d)
{
	if (GameEngine::IsDebugModeOn())
	{
...
CollisionInformation OobbCollider::CollideCheck(Collider& collidedObject)
{
	...
	case SPHERECOLLIDER:
		return CheckSphereWithOobb(collidedObject, *this);
		break;
	case BOXCOLLIDER:
		return CheckBoxWithOobb(collidedObject, *this);
	case OOBBCOLLIDER:
		return CheckOobbWithOobb(*this, collidedObject);
		break;
	case PLANECOLLIDER:
		return CheckPlaneWithOobb(collidedObject, *this);
		break;
	}
...
glm::vec3 OobbCollider::ReturnSize()
{
	return glm::vec3();
}
